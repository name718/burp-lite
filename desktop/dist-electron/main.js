...
let askpassPort = 0;
const askpassRequests = /* @__PURE__ */ new Map();
...
electron.ipcMain.handle("git:askpass-reply", (_event, reqId, password) => {
...
const gitAskpassPath = path.join(__dirname$1, "git-askpass.js");
const createGit = (cwd) => {
  return esm_default({ baseDir: cwd }).env({
    ...process.env,
...
electron.ipcMain.handle("rules:read", () => {
  try {
    const raw = require$$0$1.readFileSync(getRulesPath(), "utf-8");
...
electron.ipcMain.handle("rules:write", (_event, rules) => {
  try {
    const clean = rules.map(({ _id, ...r2 }) => r2);
    require$$0$1.writeFileSync(getRulesPath(), JSON.stringify(clean, null, 2), "utf-8");
    return { ok: true };
  } catch (err) {
    return { ok: false, msg: err.message };
  }
...
electron.ipcMain.handle("system:downloadCert", async () => {
  try {
    ...
    if (!require$$0$1.existsSync(caPath)) {
    ...
    require$$0$1.copyFileSync(caPath, filePath);
...
electron.ipcMain.handle("payload:read", (_event, id, type) => {
  try {
    ...
    const content = require$$0$1.readFileSync(filePath, "utf-8");
...
electron.ipcMain.handle("payload:write", (_event, id, type, content) => {
  try {
    const filePath = path.join("/tmp/chaos_logs", `${id}_${type}.bin`);
    require$$0$1.writeFileSync(filePath, content, "utf-8");
    return { ok: true };
  } catch (err) {
    return { ok: false, msg: err.message };
  }
...
electron.ipcMain.handle("tool:getPorts", async () => {
  try {
    ...
    uniquePorts.sort((a, b2) => a.port - b2.port);
...
electron.ipcMain.handle("project:scan", async (_event, rootDir) => {
  try {
    if (!require$$0$1.existsSync(rootDir)) return { ok: false, msg: "Directory not found" };
    const projects = [];
    const items = require$$0$1.readdirSync(rootDir);
    for (const item of items) {
      if (item === "node_modules" || item.startsWith(".")) continue;
      const itemPath = path.join(rootDir, item);
      if (require$$0$1.statSync(itemPath).isDirectory()) {
        const pkgPath = path.join(itemPath, "package.json");
        if (require$$0$1.existsSync(pkgPath)) {
          try {
            const pkgData = JSON.parse(require$$0$1.readFileSync(pkgPath, "utf-8"));
...
electron.ipcMain.handle("tool:sendCurl", async (_event, curlCmd) => {
  ...
  try {
    ...
    return { ok: false, msg: err.message, output: ((_a2 = err.stdout) == null ? void 0 : _a2.toString()) || "" };
  }
});
electron.ipcMain.handle("git:scan", async (_event, rootDir) => {
  try {
    if (!require$$0$1.existsSync(rootDir)) return { ok: false, msg: "Directory not found" };
    const gitRepos = [];
    if (require$$0$1.existsSync(path.join(rootDir, ".git"))) {
    ...
    for (const item of items) {
      ...
      const itemPath = path.join(rootDir, item);
      if (require$$0$1.statSync(itemPath).isDirectory()) {
        if (require$$0$1.existsSync(path.join(itemPath, ".git"))) {
...
electron.ipcMain.handle("git:log", async (_event, cwd) => {
  try {
    const git = createGit(cwd);
...
electron.ipcMain.handle("git:diff", async (_event, cwd, file, staged = false) => {
  try {
    const git = createGit(cwd);
    let original = "";
    try {
    ...
    let modified = "";
    if (require$$0$1.existsSync(filePath)) {
      modified = require$$0$1.readFileSync(filePath, "utf-8");
    }
...
electron.ipcMain.handle("git:stage", async (_event, cwd, file) => {
  try {
    const git = createGit(cwd);
    await git.add(file);
    return { ok: true };
...
electron.ipcMain.handle("git:unstage", async (_event, cwd, file) => {
  try {
    const git = createGit(cwd);
...
electron.ipcMain.handle("git:branch", async (_event, cwd) => {
  try {
    const git = createGit(cwd);
    const raw = await git.branch();
    const branches = raw.all ? raw.all.map((b2) => b2) : [];
    return { ok: true, branches };
  } catch (err) {
    return { ok: false, msg: err.message };
  }
});
electron.ipcMain.handle("git:checkout", async (_event, cwd, branch) => {
  try {
    const git = createGit(cwd);
...
electron.ipcMain.handle("git:discard", async (_event, cwd, file) => {
  try {
    const git = createGit(cwd);
...
electron.ipcMain.handle("git:pull", async (_event, cwd) => {
  var _a2, _b, _c;
  try {
    const git = createGit(cwd);
    const result = await git.pull();
    return { ok: true, summary: { changes: ((_a2 = result.summary) == null ? void 0 : _a2.changes) || 0, insertions: ((_b = result.summary) == null ? void 0 : _b.insertions) || 0, deletions: ((_c = result.summary) == null ? void 0 : _c.deletions) || 0 } };
  } catch (err) {
    return { ok: false, msg: err.message };
  }
});
electron.ipcMain.handle("git:push", async (_event, cwd) => {
  try {
    const git = createGit(cwd);
    await git.push();
    return { ok: true };
...
electron.ipcMain.handle("git:fetch", async (_event, cwd) => {
  try {
    const git = createGit(cwd);
    await git.fetch();
    return { ok: true };
...
electron.ipcMain.handle("git:stash", async (_event, cwd) => {
  try {
    const git = createGit(cwd);
    await git.stash();
    return { ok: true };
...
electron.ipcMain.handle("git:stashPop", async (_event, cwd) => {
  try {
    const git = createGit(cwd);
    await git.stash(["pop"]);
    return { ok: true };
...
electron.ipcMain.handle("git:resolveConflict", async (_event, cwd, file, strategy) => {
  try {
    const git = createGit(cwd);
    if (strategy === "ours") {
...
electron.ipcMain.handle("git:deleteFile", async (_event, cwd, file) => {
  try {
    const git = createGit(cwd);
    await git.rm(file);
    return { ok: true };
